"""git_utils.py — Standalone version for GitHub Manager Web (no bot dependency)"""
import os, re, shutil, zipfile, subprocess, requests
from pathlib import Path

# WORK_DIR is set by main.py after importing config
...
def replace_text(directory, old, new, only_py=True):
    ...
    return (True, "✅ Replaced in:\n" + "\n".join(f"• `{x}`" for x in changed)) if changed else (False, "❌ No occurrences found.")

def rename_folder(old_name, new_name):
    old_name, new_name = old_name.strip(), new_name.strip()
    if any(c in set('/\\:*?"<>|') for c in new_name):
        return False, "❌ Invalid name — don't use: / \\ : * ? \" < > |"
    old, new = os.path.join(WORK_DIR, old_name), os.path.join(WORK_DIR, new_name)
    if not os.path.exists(old): return False, f"❌ Folder `{old_name}` not found."
    if os.path.exists(new): return False, f"❌ `{new_name}` already exists."
...
def rename_path_in_repo(repo_folder, old_rel, new_rel):
    ...
    if not os.path.exists(repo_dir): return False, f"❌ Repo folder `{repo_folder}` not found."
    old_path, new_path = os.path.join(repo_dir, old_rel), os.path.join(repo_dir, new_rel)
    if not os.path.exists(old_path): return False, f"❌ Path `{old_rel}` not found."
    if os.path.exists(new_path): return False, f"❌ `{new_rel}` already exists."
    os.makedirs(os.path.dirname(new_path) or repo_dir, exist_ok=True)
    try: os.rename(old_path, new_path)
    except Exception as e: return False, f"❌ Rename failed: {e}"
    return True, f"✅ Renamed `{repo_folder}/{old_rel}` → `{repo_folder}/{new_rel}`"
...
def bulk_rename(repo_folder, pattern, prefix="", suffix="", replace_from="", replace_to=""):
    ...
    return (True, f"✅ Renamed {len(renamed)} file(s):\n" + "\n".join(renamed[:20])) if renamed else (False, f"❌ No files matched `{pattern}`.")

# ══ GIT OPS ════════════════════════════════════════════════════════════════════
def _env():
    return {**os.environ, "GIT_AUTHOR_NAME": "GitHub Manager", "GIT_AUTHOR_EMAIL": "bot@github.com",
            "GIT_COMMITTER_NAME": "GitHub Manager", "GIT_COMMITTER_EMAIL": "bot@github.com"}
...
def clone_repo(repo_url, dest, token=None):
    try:
        ...
        err = r.stderr.lower()
        if any(x in err for x in ["authentication", "not found", "could not read", "403", "401"]): return False, "❌ Auth failed — try with token enabled."
...
def git_push(directory, token, repo_url, branch="main"):
    try:
        ...
        run(["git", "config", "user.name",  "GitHub Manager"])
        run(["git", "add", "."])
        run(["git", "commit", "--allow-empty", "-m", f"Update via GitHub Manager"])
        run(["git", "branch", "-M", branch])
        existing = run(["git", "remote"])
        if "origin" in existing.stdout: run(["git", "remote", "set-url", "origin", auth_url])
        else: run(["git", "remote", "add", "origin", auth_url])
        r = run(["git", "push", "-u", "origin", branch, "--force"])
        os.chdir(orig)
        if r.returncode == 0: return "✅ Pushed to GitHub!"
        err = r.stderr.strip()
        if "authentication" in err.lower() or "403" in err or "401" in err:
            return "❌ Auth failed — check GitHub token has `repo` scope."
...
def git_pull(directory, token=None, repo_url=None):
    try:
        if not os.path.exists(directory): return "❌ Folder not found."
...
def unzip_and_push(zip_path, token, repo_url, branch="main"):
    ...
    try:
        ...
        if file_count == 0: return False, "❌ ZIP appears to be empty."
...
def github_create_repo(token, name, private=False, description=""):
    try:
        ...
        if r.status_code in (200, 201):
            ...
            return True, f"✅ Repo created! {'🔒 Private' if private else '🔓 Public'}", url
...
def github_delete_repo(token, repo_url):
    try:
        ...
        if r.status_code == 403: return False, "❌ Token needs `delete_repo` scope."
...
def github_merge_branch(token, repo_url, base, head, msg="Merge via GitHub Manager"):
    try:
        m = re.search(r"github\.com[:/](.+?)(?:\.git)?$", repo_url)
        if not m: return False, "❌ Invalid GitHub URL."
...
def github_update_profile(token, name=None, bio=None, location=None, blog=None, twitter_username=None):
    try:
        payload = {}
