...
# ── Config ────────────────────────────────────────────────────────────────────
from config import MONGO_URI, JWT_SECRET, APP_NAME, WORK_DIR, BOT_DIR

# Make sure workspace dir exists
os.makedirs(WORK_DIR, exist_ok=True)

# ── Bot git_utils — same exact functions the bot uses ────────────────────────
if BOT_DIR:
...
try:
    ...
    GIT_OK = True
    print(f"✅ git_utils loaded from: {BOT_DIR}")
except Exception as e:
    GIT_OK = False
    print(f"⚠️  git_utils not found: {e}")
    print("   Fix: export BOT_DIR=/home/ubuntu/GitHub/gitHub2-main")
...
def require_git():
    if not GIT_OK:
        raise HTTPException(503, "git_utils not loaded. Set BOT_DIR env var.")
